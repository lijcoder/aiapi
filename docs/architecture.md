...
- **测试文件分两类，必须能一眼区分**：
  ...
  门禁**刻意留在 `go test ./...` 里**：本仓库没有 CI，默认测试是"谁跑都全跑"的那道保证；`make gate` 只是给"提交前快速过一遍"提供的快路径。
- **预提交门禁**：`make hooks` 把 `core.hooksPath` 指向 `.githooks/`（不入库，每个 clone 各执行一次），钩子跑 `make check`，跳过只有 `--no-verify`；接线由 `TestGateGitHooksIsWired` 兜底。
