...
规则类内容还有一条额外判据：**能机械判定的，不要写成规则，写成门禁**（见 `arch_test.go` / `docs_test.go`），文档里只留一句指路。
...
**唯一权威是 `docs_test.go` 里的 `docBudgets`**（本文件不重复具体数字，避免两处漂移）。分两类处理：
...
| **目录型 / 参考型**（`api.md` 一行一个接口、`pricing.md` 描述配置结构） | 长度随产品表面线性增长，设上限等于给功能数量设上限 | **与源头一致性门禁**：`api.md` ↔ `manager/router/router.go` 的路由集合双向比对（`arch_test.go`） |
