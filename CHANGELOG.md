...
- 超管模型管理新增「复制」功能：操作列调整为「编辑」按钮 + 「更多」下拉菜单（复制/删除），点击复制后弹出新增模型弹窗并预填该模型的全部配置（provider/model 预填为 `原模型名-copy`，可修改），提交即调用已有的 `/manager/models/create` 新增模型，名称唯一性由后端校验。纯前端改动，无接口/数据结构变化。
