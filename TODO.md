...
- **要点**：Gemini 的模型名不在请求体，而在路径（如 `/v1beta/models/gemini-2.0-flash:generateContent`）。实现时除 `ParseModel` 外需实现 `ReplaceModel` 的 path 版本（用 `provider_model` 替换路径中的模型名），handler 侧需支持改写 path 而非仅 body；`ReplaceModel` 当前只覆盖请求体顶层 model，见 `parser/util/model_rewrite.go`
...
> 会随时间无限增长的表一览。统一方案：后台 goroutine + `time.Ticker`（每日）执行，
> 保留期建议做成 constant 可调；清理失败仅告警不影响服务。

| 表 | 增长速度 | 清理紧迫度 | 建议保留期 |
|---|---|---|---|
| `request_logs` | 快（全量 body） | 高 | 7~30 天 |
| `usage_records` | 中（每请求一行） | 中 | 业务定（见 17） |
| `user_sessions` | 慢 | 低 | 过期即删 |
| `recharge_records` | 慢 | **不清理** | 永久（财务凭证） |

### 17. request_logs 定期清理
- **现状**：无清理机制，全量请求/响应 body 使该表成为体积增长主力；`created_at` 无索引，按时间删除也会全表扫
- **方案**：① 先加 `created_at` 索引（存量库迁移）；② 每日定时 `DELETE FROM request_logs WHERE created_at < datetime('now','localtime','-N days')`，N 默认 30 可调；③ 与 TODO-8（body 截断/合规开关）联动，先做 8 再定保留期更稳

### 18. user_sessions 过期清理
- **现状**：启动时的 `DeleteExpired` 已移除（启动即清意义不大），过期会话目前无清理机制，长期运行会缓慢堆积
- **影响**：仅表体积膨胀，无安全风险（过期会话刷新时会被校验拒绝）
- **方案**：每日定时调用现成的 `UserSession().DeleteExpired`；若分布式改造落地（TODO-1），顺带清理 `totp_attempts`

### 19. usage_records 保留策略（需业务决策）
- **现状**：无清理机制；该表是计费与统计依据，不能直接按时间删
- **待定**：保留期定多长（半年/一年/永久？）；超期数据是删除还是聚合归档（如按天汇总到一张 archive 表，删明细留汇总，统计页长期趋势用汇总数据）
- **建议**：暂不动，量到百万级再评估

### 已完成
- `app.log`：lumberjack 轮转（100MB×10 个备份×30 天），无需额外处理
