<template>
  <div style="display:flex;flex-direction:column;gap:16px">
    <!-- 顶部指标 -->
    <n-card size="small">
      <div style="display:flex;flex-wrap:wrap;gap:12px 0">
        <div style="text-align:center;min-width:80px;flex:1">
          <div style="font-size:12px;color:#666">总请求</div>
          <div style="font-size:14px;font-weight:600;margin-top:2px">{{ summary.request_count }}</div>
        </div>
        <div style="text-align:center;min-width:80px;flex:1">
    ...
    <n-card size="small">
      <div style="display:flex;align-items:flex-end;flex-wrap:wrap;gap:12px">
        ...
        <div>
          <div style="font-size:13px;margin-bottom:4px">用户</div>
          <n-select v-model:value="query.userId" :options="filterOpts.users" placeholder="全部" clearable filterable style="width:160px" />
        </div>
        <div>
    ...
    <n-card size="small">
      <template #header>
        <n-radio-group v-model:value="query.groupBy" size="small" @update:value="doQuery">
          ...
          <n-radio-button value="user">按用户</n-radio-button>
...
import { usageStatsAdmin, usageFiltersAdmin } from '../../api'
import { fix4 } from '../../utils'
import { useChart } from '../../composables/useChart'
import { buildTimeTrendOption, buildDimensionOption } from '../../charts'
...
const filterOpts = reactive({ users: [], apiKeys: [], models: [], providers: [] })
...
async function doQuery() {
  ...
  loading.value = true
  try {
    const data = await usageStatsAdmin(
      query.mode, fmtTS(dateRange.value[0]), fmtTS(dateRange.value[1]),
      query.userId, query.apiKeyId, query.model, query.provider, query.groupBy
    )
...
async function loadFilters() {
  try {
    const data = await usageFiltersAdmin()
    filterOpts.users = (data.users || []).map(u => ({ label: `${u.name} (${u.account})`, value: u.id }))
