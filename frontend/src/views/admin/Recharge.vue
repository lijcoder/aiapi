  ...
  <n-card title="充值记录" size="small">
    <template #header-extra>
      <n-space align="center">
        <n-input v-model:value="keyword" placeholder="搜索用户/账号/备注" size="small" clearable style="width:220px" @keydown.enter="load" @clear="load" />
...
const columns = [
  ...
  { title: '用户', key: 'user_name', width: 120, ellipsis: { tooltip: true }, render(r) { return r.user_name || r.user_id } },
...
async function load() {
  ...
  try {
    records.value = (await listAllRechargeRecords(keyword.value)) || []
  } catch (e) {
